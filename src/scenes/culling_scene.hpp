...
class CullingScene final : public Scene {
public:
  ...
  void RenderInterface(int window_width, int window_height) override {
    ...
    if (ImGui::Begin("Scene Options")) {
      ...
      if (ImGui::CollapsingHeader("Culling", ImGuiTreeNodeFlags_DefaultOpen)) {
        if (ImGui::BeginCombo("Cull Face", kCullFaceModes[selected_cull_face_].name.c_str())) {
          for (auto idx = 0; idx < kCullFaceModes.size(); idx++) {
            const auto& cull_face = kCullFaceModes[idx];
            if (ImGui::Selectable(cull_face.name.c_str(), idx == selected_cull_face_)) {
              selected_cull_face_ = idx;
              glCullFace(kCullFaceModes[idx].func);
            }
          }
          ImGui::EndCombo();
        }

        if (ImGui::BeginCombo("Front Face", kFrontFaceModes[selected_front_face_].name.c_str())) {
          for (auto idx = 0; idx < kFrontFaceModes.size(); idx++) {
            const auto& front_face = kFrontFaceModes[idx];
            if (ImGui::Selectable(front_face.name.c_str(), idx == selected_front_face_)) {
              selected_front_face_ = idx;
              glFrontFace(kFrontFaceModes[idx].func);
            }
          }
          ImGui::EndCombo();
        }
